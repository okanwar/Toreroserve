 ...
 * the argument is really a (ThreadArg *), the first thing we do is cast
 * it to that type, to make it usable. */
void *thread_function(void *argument_value) {
    ThreadArg *my_argument = (ThreadArg *) argument_value;
...
int main(int argc, char **argv) {
    ...
    ThreadArg *arguments = malloc(num_threads * sizeof(ThreadArg));
    if (arguments == NULL) {
